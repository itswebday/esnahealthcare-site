...
/*
  Hero variant 10 — "Atmospheric Mesh"
  Multiple soft radial gradient blobs blend inside the box to create a
  cloud-like atmospheric mesh — primary top, accent bottom, with a
  centred highlight. Heavy blur for dimension. No text. Soft, premium.
*/

const HeroTenPage: React.FC = () => {
  return (
    <>
      <PreviewBanner label="Hero 10 · Atmospheric Mesh" />
      <Hero />
    </>
  );
};
...
const BoxBackdrop: React.FC = () => {
  return (
    <div
      aria-hidden="true"
      className="from-primary/18 to-accent/18 pointer-events-none absolute -inset-x-5 -top-[200px] -bottom-8 rounded-3xl bg-linear-to-br sm:-inset-x-6 sm:-top-[220px] sm:-bottom-10 lg:-inset-y-4 lg:right-[-40vw] lg:-left-[45%]"
    />
  );
};
